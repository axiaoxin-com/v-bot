...
func NewDB(host string, port int, user, passwd string) (*sqlx.DB, error) {
	...
	log.Println("[DEBUG] cnarea mysql dsn:", mydsn)
...
func Province(db *sqlx.DB, shortName string) (Area, error) {
	area := Area{}
	if err := db.Get(&area, "select * from cnarea_2018 where level = 0 and short_name = ?", shortName); err != nil {
		return area, err
	}
	return area, nil
}
...
func Provinces(db *sqlx.DB) ([]Area, error) {
	areas := []Area{}
	if err := db.Select(&areas, "select * from cnarea_2018 where level = 0"); err != nil {
		return nil, err
	}
	return areas, nil
}
...
func City(db *sqlx.DB, shortName string) (Area, error) {
	area := Area{}
	if err := db.Get(&area, "select * from cnarea_2018 where level = 1 and short_name = ?", shortName); err != nil {
		return area, err
	}
	return area, nil
}
...
func Cities(db *sqlx.DB, proviceShortName string) ([]Area, error) {
	...
	if err := db.Select(&areas, "select * from cnarea_2018 where level = 1 and parent_code = ?", provinceArea.AreaCode); err != nil {
		return nil, err
	}
	return areas, nil
}
...
func Districts(db *sqlx.DB, cityShortName string) ([]Area, error) {
	...
	if err := db.Select(&areas, "select * from cnarea_2018 where level = 2 and parent_code = ?", cityArea.AreaCode); err != nil {
		return nil, err
	}
	return areas, nil
}
